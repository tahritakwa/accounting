...
@Service("AccountCustomerService")
public class AccountCustomerService extends GenericService<AccountCustomer, Long>
        implements IGenericAccountRelationService {
    private final AccountService accountService;
    private final AccountCustomerDao accountCustomerDao;

    @Autowired
    public AccountCustomerService(AccountService accountService, AccountCustomerDao accountCustomerDao) {
    ...
    public GenericAccountRelationDto saveAccountRelation(GenericAccountRelationDto accountRelationDto) {
        ...
        if (existingAccountWithRelation != null
            ...
            throw new HttpCustomException(ApiErrors.Accounting.ACCOUNT_RELATION_TYPE_DUPLICATES, new ErrorsResponse()
                    .error(accountRelationDto.getRelationEntityId()).error(AccountRelationType.CUSTOMER));
        } else {
            if (account != null) {
                return GenericAccountRelationConverter
                        .modelToDto(saveAndFlush(new AccountCustomer(accountRelationDto.getId(), account,
    ...
    public Optional<GenericAccountRelation> findAccountWithRelation(Long relationEntityId) {
        return accountCustomerDao.findByRelationEntityIdAndIsDeletedFalse(relationEntityId);
    ...
    public List<GenericAccountRelationDto> findAllEntitiesInRelationWithAccount(Long accountId) {
        return GenericAccountRelationConverter
                .modelsToDtos(accountCustomerDao.findAllByAccountIdAndIsDeletedFalse(accountId));
    }
